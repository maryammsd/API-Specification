...
def check_similarity(json_result,excel_result,node_value):
    ...
    response = prompt.run_llm_model(prompt_)
...
def check_no_setting(response):
    ...
    if "no further device setting" in response_lower:
        return True
    if "no further Android device setting" in response_lower:
        return True
    if "no specific permission" in response_lower:
        return True
...
def compare_json_with_excel(json_file_path, excel_file_path):
    ...
    try:
        ...
        matching_elements = [item for item in json_data]
        ...
        for item in matching_elements:
            ...
            temp = f"{parent_value}.{node_value}"
            if temp not in items_list:
            ...
            if str(parent_value) != "":
                ...
                matching_row = excel_data[excel_data.iloc[:, 0] == parent_value]  # First column contains node values
                value = 0
                ...
                if not matching_row.empty:
                    ...
                    generated_result = prompt.create_prompt_class_our_solution(node_value,"35")
                    response = prompt.run_llm_model(generated_result)
        ...
        print(f" Total number of methods {method_count}")
    ...
    with open(file_path,"r") as file:
        for line in file:
            ...
            if "INFO -" in line:
                ...
                if len(values) >= 6 and "[Static Analysis]" not in str(values) and "[Our Solution]" not in str(values):
...
def analyze_graph_data(log_file_path):

    # Initialize lists to store nodes and edges values
    nodes_list = []
    edges_list = []
    ...
    with open(log_file_path, "r") as file:
        for line in file:
            # Check if the line contains "nodes" and "edges"
            if "nodes" in line and "edges" in line:
                # Extract the numbers for nodes and edges using regex
                match = re.search(r"(\d+)\s+nodes.*?(\d+)\s+edges", line)
                if match:
        ...
        # Check if the line contains "nodes" and "edges"
            if "nodes" in line and "edges" in line:
                # Extract the numbers for nodes and edges using regex
                match = re.search(r"nodes\s*=\s*(\d+).*edges\s*=\s*(\d+)", line)
                if match:
                    nodes = int(match.group(1))
                    edges = int(match.group(2))
                    nodes_list.append(nodes)
    ...
    if nodes_list:
        ...
        nodes_median = statistics.median(nodes_list)
        nodes_average = statistics.mean(nodes_list)
    else:
        nodes_min = nodes_max = nodes_median = nodes_average = None

    # Calculate statistics for edges
    if edges_list:
        edges_min = min(edges_list)
        edges_max = max(edges_list)
        edges_median = statistics.median(edges_list)
        edges_average = statistics.mean(edges_list)
    else:
        edges_min = edges_max = edges_median = edges_average = None

    # Print statistics for nodes
    print("Statistics for Nodes:")
    if nodes_min is not None:
        print(f"Min: {nodes_min}, Max: {nodes_max}, Median: {nodes_median}, Average: {nodes_average:.2f}")
    else:
        print("No data available for nodes.")

    # Print statistics for edges
    print("\nStatistics for Edges:")
    if edges_min is not None:
        print(f"Min: {edges_min}, Max: {edges_max}, Median: {edges_median}, Average: {edges_average:.2f}")
    else:
        print("No data available for edges.")

# Example usage
json_file_path = "/home/maryam/clearblue/java-code/py-code/response_log_gpt_new.json"
...
app_log_file_path =  "/home/maryam/clearblue/java-code/py-code/application.log"
#compare_json_with_excel(json_file_path, excel_file_path)
#read_result_logs(log_file_path)
analyze_graph_data(app_log_file_path)